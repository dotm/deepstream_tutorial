...
class RPC extends Component {
    constructor(props) {
        ...
        this.rpc.provide('multiply-number', function( data, response ){
            // respond to the request by multiplying the incoming number with the one from the response input
            response.send(data.value * parseFloat(this.state.responseValue));
        }.bind(this));
    }

    handleClick = (e) => {
        // read the value from the input field and convert it into a number
        var data = {
            value: parseFloat(this.state.requestValue)
        };

        // Make a request for `multiply-number` with our data object and wait for the response
